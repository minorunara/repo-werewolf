...
- 生存者が多い時は会議後にランダムな組へ組み替え、マップ各所へ分散スポーンさせる
- 野良部屋などで「3番が怪しい」のように使える参加者ID
...
組分けは全員に発表されますが、どの組がどこへ行くのかは分かりません。  
前回の組分けは、次の会議の冒頭に会議チャットログで再掲されます。  
...
処刑するほどの確証がない相手は、武器を没収して様子を見るのも一つの手です。  
...
- **無効試合**: バグ・切断・事故（役職が透けてしまった等）で試合の続行が不可能になったとき、ホストは`VoidMatchKey`の長押しで試合を打ち切れます。確認画面が出てから改めて長押しすると確定します（誤操作を防ぐため二段階です）。勝者はつかず、コスメティックトークンも配布されませんが、結果画面と試合の記録は通常どおり表示され、全員の役職も公開されます

## 配信者向けセーフモード

一部のパロディ表現や特徴的な効果音を念のため汎用素材・無音に置き換えるモードです。  
設定ファイル（`BepInEx/config/minorunara.werewolf.cfg`）の `[Streamer]` セクションで `StreamerSafeMode = true` にしてください（反映はゲーム再起動）:  

- 爆弾魔の能力アイコン2種（パロディ表現）→ 汎用の爆弾アイコン
- 会議招集のチャイム → 汎用の通知音
- 処刑演出の合の手2種 → 無音（この合の手自体はフリー素材です〔「利用素材」のAfrican Mist Voice参照〕が、有名ゲームでの使用例が知られているため対象に含めています）

ゲームの進行・演出のタイミングは変わりません。  
この設定は各プレイヤーのローカル設定で、自分の画面と音にだけ効くので配信者本人がONにすれば十分です。
